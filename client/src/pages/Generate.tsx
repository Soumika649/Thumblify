...
const Generate = () => {
    ...
    const fetchThumbnail = async () => {
        ...
        try {
            ...
            // Stop loading only when image_url exists
            if (data?.thumbnail?.image_url) setLoading(false);
    ...
    useEffect(() => {
        ...
        fetchThumbnail(); // fetch immediately

        const interval = setInterval(fetchThumbnail, 3000); // poll every 3 seconds
        return () => clearInterval(interval);
    }, [id, isLoggedIn]);

    // Reset thumbnail when navigating away
    useEffect(() => {
                            ...
                            <div className='p-6 rounded-2xl bg-white/8 border border-white/12 shadow-xl space-y-6'>
                                ...
                                {!id && (
                                    <button
                                        ...
                                        disabled={loading}
                                        className='text-[15px] w-full py-3.5 rounded-xl font-medium bg-linear-to-b from-pink-500 to bg-pink-600 hover:from-pink-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors'
                                    >
